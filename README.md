# Paper-List-of-MARL
A new paper list for multi-agent reinforcement learning (actively maintained), collecting papers from 2018.

Todo: AAMAS 2018 and AAMAS 2019

## ICLR 2019

- [Probabilistic Recursive Reasoning for Multi-Agent Reinforcement Learning](https://openreview.net/forum?id=rkl6As0cF7)

- [Learning to Schedule Communication in Multi-agent Reinforcement Learning](https://openreview.net/forum?id=SJxu5iR9KQ)
- [M^3RL: Mind-aware Multi-agent Management Reinforcement Learning](https://openreview.net/forum?id=BkzeUiRcY7)
- [Stochastic Prediction of Multi-Agent Interactions from Partial Observations](https://openreview.net/forum?id=r1xdH3CcKX)
- [Learning when to Communicate at Scale in Multiagent Cooperative and Competitive Tasks](https://openreview.net/forum?id=rye7knCqK7)
- [Relational Forward Models for Multi-Agent Learning](https://openreview.net/forum?id=rJlEojAqFm)
- [TarMAC: Targeted Multi-Agent Communication](https://openreview.net/forum?id=H1e572A5tQ)



## ICML 2019

- [Bayesian Action Decoder for Deep Multi-Agent Reinforcement Learning](https://arxiv.org/abs/1811.01458?source=post_page---------------------------)
- [Multi-Agent Adversarial Inverse Reinforcement Learning](http://proceedings.mlr.press/v97/yu19e.html)

- [Learning to Collaborate in Markov Decision Processes](https://arxiv.org/abs/1901.08029?source=post_page---------------------------)
- [Actor-Attention-Critic for Multi-Agent Reinforcement Learning](https://arxiv.org/abs/1810.02912)
- [Social Influence as Intrinsic Motivation for Multi-Agent Deep Reinforcement Learning](https://arxiv.org/abs/1810.08647?source=post_page---------------------------)
- [TarMAC: Targeted Multi-Agent Communication](https://arxiv.org/abs/1810.11187)

- [Finite-Time Analysis of Distributed TD(0) with Linear Function Approximation on Multi-Agent Reinforcement Learning](http://proceedings.mlr.press/v97/doan19a/doan19a.pdf)

- [Grid-Wise Control for Multi-Agent Reinforcement Learning in Video Game AI](http://proceedings.mlr.press/v97/han19a/han19a.pdf)

- [QTRAN: Learning to Factorize with Transformation for Cooperative Multi-Agent Reinforcement Learning](http://lanada.kaist.ac.kr/Publication/Conference/QTRAN.pdf)



## NIPS 2018

- [Learning others’ intentional models in multi-agent settings using interactive POMDPs](https://papers.nips.cc/paper/7806-learning-others-intentional-models-in-multi-agent-settings-using-interactive-pomdps.pdf)
- [Inequity aversion improves cooperation in intertemporal social dilemmas](https://papers.nips.cc/paper/7593-inequity-aversion-improves-cooperation-in-intertemporal-social-dilemmas.pdf)
- [Learning attentional communication for multi-agent cooperation](https://arxiv.org/abs/1805.07733)
- [Actor-critic policy optimization in partially observable multiagent environments](https://arxiv.org/abs/1810.09026)
- [Credit assignment for collective multiagent RL with global rewards](https://papers.nips.cc/paper/8033-credit-assignment-for-collective-multiagent-rl-with-global-rewards.pdf)
- [Multi-agent generative adversarial imitation learning](https://arxiv.org/abs/1807.09936)
- [Multi-agent reinforcement learning via double averaging primal-dual optimization](https://arxiv.org/abs/1806.00877)
- [A deep Bayesian policy reuse approach against non-stationary agents](https://papers.nips.cc/paper/7374-a-deep-bayesian-policy-reuse-approach-against-non-stationary-agents)
- [Multi-agent online learning with asynchronous feedback loss](https://pdfs.semanticscholar.org/f7b4/e31d9c01edd1a0d64047db341d70d4dc7568.pdf)



## ICML 2018

- [Competitive Multi-agentInverse Reinforcement Learning with Sub-optimal Demonstrations](https://arxiv.org/abs/1801.02124)
- [Learning to Act inDecentralized Partially Observable MDPs](http://proceedings.mlr.press/v80/dibangoye18a.html)
- [Learning PolicyRepresentations in Multiagent Systems](https://arxiv.org/abs/1806.06464)
- [Mean Field Multi-Agent Reinforcement Learning](https://arxiv.org/pdf/1802.05438.pdf)
- [QMIX: Monotonic Value Function Factorisation for Deep Multi-Agent Reinforcement Learning](https://arxiv.org/pdf/1803.11485.pdf)

- [Modeling Others using Oneself in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/1802.09640)



## ICLR 2018

- [Emergent Translation in Multi-Agent Communication](https://arxiv.org/abs/1710.06922)

- [Emergent Communication through Negotiation](https://arxiv.org/abs/1804.03980)

- [Learning Deep Mean Field Games for Modeling Large Population Behavior](https://arxiv.org/abs/1711.03156)

